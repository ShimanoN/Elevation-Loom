function drawWeeklyChart(canvasId, weekData, weekTarget) {
    ...
    const fontFamily = "'Avenir Next', 'Avenir', 'Futura', 'Gill Sans', 'Trebuchet MS', sans-serif";
    ...
    const maxDaily = Math.max(0, ...data.map(d => Math.max(d.plan, d.actual || 0)));
    const maxCumulative = Math.max(0, cumulativePlan, cumulativeActual, weekTarget || 0);
    ...
    data.forEach((d, i) => {
        ...
        if (d.plan > 0) {
            ...
            drawRoundedRect(ctx, xCenter - barWidth - 2, baseY - barHeight, barWidth, barHeight, 6, palette.planFill, palette.planStroke);
        }

        if (d.actual !== null && d.actual > 0) {
            const barHeight = (d.actual / yMaxLeft) * chartHeight;
            drawRoundedRect(ctx, xCenter + 2, baseY - barHeight, barWidth, barHeight, 6, palette.actualFill, palette.actualStroke);
        }
    ...
    drawCumulativeLine(ctx, data, padding, chartHeight, width, height, categoryWidth, yMaxRight, 'cumPlan', palette.planLine, true);
    drawCumulativeLine(ctx, data.filter(d => d.cumActual !== null), padding, chartHeight, width, height, categoryWidth, yMaxRight, 'cumActual', palette.actualLine, false);
...
function drawCumulativeLine(ctx, data, padding, chartHeight, width, height, categoryWidth, yMaxRight, valueKey, color, dashed) {
...
function drawLegend(ctx, width, padding, palette, fontFamily) {
    const legendTop = 18;
    const items = [
        { label: 'Plan (Daily)', type: 'bar', fill: palette.planFill, stroke: palette.planStroke },
        { label: 'Actual (Daily)', type: 'bar', fill: palette.actualFill, stroke: palette.actualStroke },
        { label: 'Plan (Cum)', type: 'line', color: palette.planLine, dashed: true },
        { label: 'Actual (Cum)', type: 'line', color: palette.actualLine, dashed: false },
