...
- **日付ナビゲーションの軽微な不整合（TODO）**: `app.js` の `changeDate` 関数内に `remainingPastDays` と `updateNavigationState` への参照が残っていますが、これらは定義されていません。UI上は `updateNavButtons()` による30日制限の無効化が働きますが、日付変更関数の内部実装には未完のロジックがあるため、挙動確認と修正が推奨されます。
